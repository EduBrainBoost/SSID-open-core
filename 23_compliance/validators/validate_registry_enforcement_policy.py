...
def validate_registry_enforcement_policy(data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    for artifact in artifacts:
        ...
        if artifact.get("on_disk") is True and not artifact.get("hash_sha256"):
            violations.append(
                f"REGISTRY_ENFORCE_001: artifact '{path}' on_disk without hash_sha256"
            )

        # REGISTRY_ENFORCE_002: hash drift
        if (artifact.get("on_disk") is True
                and artifact.get("hash_sha256")
                and artifact.get("disk_hash")):
            if artifact["hash_sha256"] != artifact["disk_hash"]:
                violations.append(
                    f"REGISTRY_ENFORCE_002: hash drift for '{path}'"
                )

        # REGISTRY_ENFORCE_003: missing or empty evidence_ref
        if not artifact.get("evidence_ref"):
            violations.append(
                f"REGISTRY_ENFORCE_003: missing evidence_ref for '{path}'"
            )
        ...
        if _is_sot_artifact(artifact) and not artifact.get("source_of_truth_ref"):
            violations.append(
                f"REGISTRY_ENFORCE_004: SoT artifact '{path}' without source_of_truth_ref"
            )
    ...
    for guard in guards:
        ...
        if guard.get("unknown_value_behavior") != "fail":
            violations.append(
                f"REGISTRY_ENFORCE_005: guard '{guard.get('name', '<unknown>')}' has fail-open behavior"
            )
