...
ROOT_DESCRIPTIONS = {
    "01_ai_layer": {"name": "AI/ML & Intelligenz", "purpose": "KI-Modelle, Risk/Trust-Scoring, Bias-/Fairness-Kontrollen, AI Governance (EU AI Act)"},
    "02_audit_logging": {"name": "Nachweise & Beweisfuehrung", "purpose": "Hash-Ledger, Evidence-Matrix, Blockchain Anchors, Audit-Trails, Retention"},
    "03_core": {"name": "Zentrale Logik", "purpose": "Smart Contract Kernsystem, Dispatcher, Identity-Resolver, Root-24-LOCK Enforcement"},
    "04_deployment": {"name": "Auslieferung & Distribution", "purpose": "CI/CD-Pipelines, Rollouts, Container, K8s, Terraform"},
    "05_documentation": {"name": "Dokumentation & I18N", "purpose": "Developer Guides, API Docs, Mehrsprachigkeit, Docusaurus"},
    "06_data_pipeline": {"name": "Datenfluss & Verarbeitung", "purpose": "ETL/ELT-Prozesse, Batch/Stream/Realtime, ML/AI Data-Feeds"},
    "07_governance_legal": {"name": "Recht & Steuerung", "purpose": "eIDAS, MiCA, DSGVO, DORA, DAO-Governance-Regeln"},
    "08_identity_score": {"name": "Reputation & Scoring", "purpose": "Identity Trust Levels, Scoring-Algorithmen, Verhaltensanalysen (hash-only)"},
    "09_meta_identity": {"name": "Digitale Identitaet", "purpose": "DID-Schemas, Identity Wallets, Selective Disclosure, Lifecycle"},
    "10_interoperability": {"name": "Kompatibilitaet", "purpose": "DID-Resolver, Cross-Chain Bridges, Protokoll-Adapter, API-Gateways"},
    "11_test_simulation": {"name": "Simulation & QA", "purpose": "Testumgebungen, Mock-Chains, Chaos Engineering, Benchmarking"},
    "12_tooling": {"name": "Werkzeuge", "purpose": "CLI, SDKs, CI-Helper, Linter, Automation Scripts"},
    "13_ui_layer": {"name": "Benutzeroberflaeche", "purpose": "Frontend, Dashboards, Partner-/User-Portale, Admin-GUIs"},
    "14_zero_time_auth": {"name": "Sofort-Authentifizierung", "purpose": "Real-Time KYC/KYB, Zero-Time Login, Biometrie, MFA, DID-Sessions"},
    ...
    "18_data_layer": {"name": "Datenhaltung", "purpose": "Datenbanken, GraphDBs, Time-Series, Encryption-at-Rest, Backups"},
    "19_adapters": {"name": "Anschluesse & Schnittstellen", "purpose": "Adapter zu externen APIs/Chains, Payment-Provider, Identity SDKs"},
    "20_foundation": {"name": "Grundlagen & Tokenomics", "purpose": "SSID-Token, Tokenomics, Distribution, NFT-Lizenzen, DAO-Treasury"},
    "21_post_quantum_crypto": {"name": "Zukunftskrypto", "purpose": "PQC-Algorithmen (Kyber, Dilithium), Quantum-Safe Migration"},
    ...
    "24_meta_orchestration": {"name": "Zentrale Steuerung", "purpose": "Dispatcher, Registry, Locks, Trigger, Gates, Global Hash-Ledger"},
}

SHARD_DESCRIPTIONS = {
    "01_identitaet_personen": {"name": "Identitaet & Personen", "scope": "DIDs, Ausweise, Profile, Authentifizierung, Personen, Firmen, Organisationen"},
    "02_dokumente_nachweise": {"name": "Dokumente & Nachweise", "scope": "Urkunden, Bescheinigungen, Zertifikate, Vollmachten, digitale Signaturen"},
    "03_zugang_berechtigungen": {"name": "Zugang & Berechtigungen", "scope": "Rollen, Rechte, Mandanten, Delegationen, MFA, Zero-Trust, Sessions"},
    "04_kommunikation_daten": {"name": "Kommunikation & Daten", "scope": "Nachrichten, E-Mail, Chat, Datenaustausch, APIs, Benachrichtigungen"},
    "05_gesundheit_medizin": {"name": "Gesundheit & Medizin", "scope": "Krankenakte, Rezepte, Impfpass, Behandlungen, Kliniken, Apotheken"},
    "06_bildung_qualifikationen": {"name": "Bildung & Qualifikationen", "scope": "Zeugnisse, Abschluesse, Kurse, Weiterbildung, Zertifizierungen"},
    "07_familie_soziales": {"name": "Familie & Soziales", "scope": "Geburt, Heirat, Scheidung, Erbe, Vormundschaft, Sozialleistungen, Vereine"},
    "08_mobilitaet_fahrzeuge": {"name": "Mobilitaet & Fahrzeuge", "scope": "Fuehrerschein, KFZ-Zulassung, Fahrzeugpapiere, Maut, Versicherung"},
    "09_arbeit_karriere": {"name": "Arbeit & Karriere", "scope": "Arbeitsvertraege, Gehalt, Bewerbungen, Referenzen, Freelancing"},
    "10_finanzen_banking": {"name": "Finanzen & Banking", "scope": "Konten, Zahlungen, Kredite, Investments, DeFi, Krypto, Abonnements"},
    "11_versicherungen_risiken": {"name": "Versicherungen & Risiken", "scope": "Alle Versicherungsarten, Schaeden, Claims, Policen, Praemien"},
    "12_immobilien_grundstuecke": {"name": "Immobilien & Grundstuecke", "scope": "Eigentum, Miete, Pacht, Grundbuch, Hypotheken, Bewertungen"},
    "13_unternehmen_gewerbe": {"name": "Unternehmen & Gewerbe", "scope": "Firmendaten, Handelsregister, Lizenzen, B2B, Buchhaltung, Bilanzen"},
    "14_vertraege_vereinbarungen": {"name": "Vertraege & Vereinbarungen", "scope": "Smart Contracts, Geschaeftsvertraege, AGBs, SLAs, Partnerschaften"},
    "15_handel_transaktionen": {"name": "Handel & Transaktionen", "scope": "Kaeufe, Verkaeufe, Rechnungen, Garantien, Supply Chain, Logistik"},
    "16_behoerden_verwaltung": {"name": "Behoerden & Verwaltung", "scope": "Aemter, Antraege, Genehmigungen, Steuern, Meldewesen, Gerichtsurteile"},
}
...
def add_missing_fields(chart_path, root_id, shard_id):
    ...
    for line in content.split('\n'):
        stripped = line.strip()
        if ':' in stripped and not stripped.startswith('-') and not stripped.startswith('#') and not line.startswith(' ') and not line.startswith('\t'):
    ...
    return {
        ...
        "fields_added": len(additions)
    }

def main():
    roots = sorted([d for d in os.listdir(SSID_ROOT) if os.path.isdir(os.path.join(SSID_ROOT, d)) and d[:2].isdigit() and '_' in d])
