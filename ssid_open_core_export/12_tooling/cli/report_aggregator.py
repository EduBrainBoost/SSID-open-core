...
_REPORT_REQUIRED = ("run_id", "timestamp_utc", "pipeline_version", "decision", "summary", "findings")
SEVERITY_ORDER = {"deny": 0, "warn": 1, "info": 2}
REQUIRED_KEYS = {"id", "class", "severity", "source", "path", "details", "timestamp", "repo"}
RUN_IDENTITY_KEYS = {"run_id", "timestamp_utc", "contract_sha256", "canonical_commit", "derivative_commit", "decision"}
...
def write_audit_event(
    output_dir: Path, run_id: str, ts: str, status: str, stats: dict, report_sha256: str, evidence_sha256: str
) -> Path:
...
def render_markdown(
    ...
    if sorted_findings:
        lines += [
            "| # | Severity | Class | Source | Repo | Path | Details |",
            "|---|----------|-------|--------|------|------|---------|",
        ]
        for idx, f in enumerate(sorted_findings, 1):
            lines.append(
                f"| {idx} | {f['severity']} | {f['class']} | {f['source']} "
    ...
    lines += [
        "## Policy Evaluation",
        "",
        f"- Deny rules fired: {deny_n}",
        f"- Warn rules fired: {warn_n}",
        f"- Info rules fired: {info_n}",
        f"- Compliant: {status == 'PASS'}",
        "",
        "---",
        "",
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--run-identity", type=Path, default=None, help="Optional JSON file with run identity metadata")
    ...
    report_payload = build_json_report(
        args.run_id, findings, stats, status, ts, run_identity=run_identity, evidence_sha256=evidence_sha
    )
    ...
    # Audit event
    audit_out = write_audit_event(args.output_dir, args.run_id, ts, status, stats, report_hash, evidence_sha)
    # Markdown report
    md_text = render_markdown(
        args.run_id, findings, stats, status, ts, report_hash, evidence_sha, run_identity=run_identity
    )
    md_out = args.output_dir / "sot_convergence_report.md"
    md_out.write_text(md_text, encoding="utf-8")
    print(
        f"[{status}] {stats['total']} findings aggregated  (deny={stats['by_severity'].get('deny', 0)}, "
