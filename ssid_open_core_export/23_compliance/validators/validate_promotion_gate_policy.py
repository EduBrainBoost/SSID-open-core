...
def validate_promotion_gate_policy(data: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    for c in canonical_artifacts:
        if isinstance(c, dict) and c.get("on_disk") is True and c.get("path") not in derivative_paths:
            violations.append(f"PROMO_ENFORCE_001: canonical '{c.get('path')}' on_disk but no derivative counterpart")

    # PROMO_ENFORCE_002: derivative with no canonical source
    for d in derivative_artifacts:
        if isinstance(d, dict) and d.get("path") not in canonical_paths:
            violations.append(f"PROMO_ENFORCE_002: derivative '{d.get('path')}' has no canonical source")
    ...
    for d in derivative_artifacts:
        if isinstance(d, dict) and d.get("path") in canonical_by_path:
            ...
            if c.get("hash_sha256") and d.get("hash_sha256") and c["hash_sha256"] != d["hash_sha256"]:
                violations.append(f"PROMO_ENFORCE_004: hash drift for '{d.get('path')}'")
    ...
    for d in derivative_artifacts:
        if isinstance(d, dict):
            ...
            if not any(path.startswith(scope) for scope in export_scopes):
                violations.append(f"PROMO_ENFORCE_005: derivative '{path}' outside allowed export scopes")
