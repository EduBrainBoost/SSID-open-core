...
class OversightGraph:
    ...
    def get_watchdog(self, agent_id: str) -> Optional[str]:
...
def validate_no_unwatched(graph: OversightGraph) -> bool:
    ...
    for agent_id, assignment in graph.assignments.items():
        ...
        if assignment.watchdog_agent_id != agent_id:
            if assignment.watchdog_agent_id not in graph.assignments:
                return False
    return True
    ...
    while True:
        ...
        if watchdog == current:
            ...
            if current != L0_SENTINEL:
                raise ValueError(
                    f"FAIL-CLOSED: Non-L0 agent '{current}' has self-referencing watchdog"
                )
            break

        if watchdog in visited:
            raise ValueError(
                f"FAIL-CLOSED: Cycle detected in escalation chain at '{watchdog}'"
            )
