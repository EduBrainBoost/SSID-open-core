...
from .incident_classifier import Incident, INCIDENT_TYPES
...
def containment(incident: Incident) -> Incident:
    ...
    action = _CONTAINMENT_ACTIONS.get(
        incident.incident_type, "full_isolation_unknown_type"
    )
    incident.containment_status = "contained"
    incident.description = (
        f"{incident.description or ''} | CONTAINMENT: {action}"
    ).lstrip(" |")
    return incident
...
def diagnose(incident: Incident) -> Incident:
    ...
    hint = _DIAGNOSIS_HINTS.get(
        incident.incident_type, "unknown_type_full_diagnostic_required"
    )
...
def repair(incident: Incident) -> Incident:
    ...
    action = _REPAIR_ACTIONS.get(
        incident.incident_type, "manual_intervention_required"
    )
