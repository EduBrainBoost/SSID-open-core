...
from ssid_security.secret_patterns import redact as _redact_secrets, find as _find_secrets
# -- End bootstrap ----------------------------------------------------------
...
class SSIDRedactor:
    ...
    def __init__(self, log_mode: str = "MINIMAL"):
        ...
        self.redaction_stats = {
            "total_redactions": 0,
            "rules_applied": {rule.name: 0 for rule in self.REDACTION_RULES}
        }
    ...
    def create_safe_summary(self, full_text: str, max_lines: int = 20) -> str:
        ...
        summary_lines = lines[:max_lines//2] + [f"... [{len(lines) - max_lines} lines omitted] ..."] + lines[-(max_lines//2):]
        return "\\n".join(summary_lines)
    ...
    def get_redaction_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "redaction_timestamp": datetime.datetime.now(datetime.UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
...
class SSIDDataMinimizer:
    ...
    def create_minimal_evidence(self, task_id: str, patch_content: str, 
    ...
    def process_agent_output(self, stdout: str, stderr: str, 
                           prompt: str = "") -> Tuple[str, str, Optional[str]]:
        ...
        if prompt:
            ...
            if self.should_persist_prompt():
                ...
                processed_prompt = self.redactor.redact_text(prompt)
