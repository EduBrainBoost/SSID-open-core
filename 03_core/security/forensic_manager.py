...
class SSIDForensicManager:
    ...
    def __init__(self, repo_root: Path, config: Optional[ForensicConfig] = None):
    ...
    def _is_expired(self, evidence_dir: Path) -> bool:
        ...
        try:
            ...
            expiry_time = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=self.config.max_retention_hours)
    ...
    def store_forensic_evidence(self, task_id: str, original_evidence: Path, 
                               prompt: str = "", stdout: str = "", stderr: str = "") -> Path:
        ...
        forensic_data = {
            ...
            "expires_utc": (datetime.datetime.now(datetime.UTC) + datetime.timedelta(hours=self.config.max_retention_hours)).isoformat().replace("+00:00", "Z"),
        ...
        for item in self.forensic_root.iterdir():
            if item.name.startswith("forensic_"):
                try:
                    ...
                    manifest_path = item / "forensic_manifest.json" if item.is_dir() else item.with_suffix(".tar.gz").parent / "forensic_manifest.json"
                        ...
                        if item.suffix == ".gz":
                            ...
                            try:
                                result = subprocess.run([
                                    "tar", "-xzf", str(item), 
                                    "--to-stdout", "forensic_manifest.json"
                                ], capture_output=True, text=True, cwd=str(item.parent))
...
def main() -> int:
    ...
    config = ForensicConfig(max_retention_hours=args.max_hours, evidence_root=repo_root / "02_audit_logging" / "evidence")
