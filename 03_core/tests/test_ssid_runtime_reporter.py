...
from ssid_runtime_reporter import SsidRuntimeReporter, SsidRuntimeReport
...
def test_all_flows_succeed():
    ...
    for flow in report.flow_statuses:
        assert flow.get("status") == "success", (
            f"Flow {flow.get('flow_name')} has status {flow.get('status')!r}, "
            f"error: {flow.get('error', 'n/a')}"
        )
...
def test_module_health_has_8_entries():
    ...
    for entry in report.module_health:
        assert entry["status"] == "healthy", (
            f"Module {entry['module_name']} is not healthy: {entry['status']!r}"
        )
...
def test_flow_statuses_have_proof_hash():
    ...
    for flow in report.flow_statuses:
        ...
        assert proof_hash is not None, (
            f"Flow {flow.get('flow_name')} has no proof_hash"
        )
