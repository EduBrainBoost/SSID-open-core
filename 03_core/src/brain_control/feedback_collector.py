...
class FeedbackCollector:
    ...
    def collect(self, session_id: str, interaction_id: str, rating: int, comment: str = "", metadata: Optional[Dict] = None) -> str:
        fb_id = f"fb_{uuid.uuid4().hex[:12]}"
        entry = {"feedback_id": fb_id, "session_id": session_id, "interaction_id": interaction_id, "rating": max(1, min(5, rating)), "comment": comment, "metadata": metadata or {}, "timestamp": time.time()}
