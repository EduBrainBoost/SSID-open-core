...
class ApprovalGate:
    def __init__(self, auto_approve: Optional[List[str]] = None):
    ...
    def check(self, action: str, scope: str = "", role: str = "user") -> GateResult:
        ...
        if action in CRITICAL_ACTIONS:
            ...
            return GateResult(action=action, decision=GateDecision.DENIED, reason=f"critical action '{action}' requires admin role", requires_human=True)
        return GateResult(action=action, decision=GateDecision.REQUIRES_REVIEW, reason="unknown action type", requires_human=True)
