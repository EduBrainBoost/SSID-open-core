...
def test_canonical_pass(
    _mock_git, _mock_scanner, _mock_render, _mock_manifest, _mock_oc, tmp_repos
):
...
def test_derivative_leakage_fail():
    ...
    scan = _clean_scan()
    sync = _clean_sync(status="fail", findings=[
        {"class": "forbidden_export", "severity": "critical",
...
def test_contract_hash_mismatch_fail():
    ...
    scan = _clean_scan()
    sync = _clean_sync(status="fail", findings=[
        {"class": "contract_hash_mismatch", "severity": "critical",
...
def test_deterministic_aggregator_output():
    ...
    scan = _clean_scan(drift=[
        {"class": "stale_derivative_binding", "severity": "medium",
         "path": "03_core/bindings.json", "detail": "stale hash"},
    ])
    sync = _clean_sync(findings=[
        {"class": "missing_expected_export", "severity": "medium",
         "path": "docs/overview.md", "detail": "not exported"},
    ])
...
def test_ems_publish_payload_validity(tmp_repos):
    ...
    for key in ("run_id", "timestamp_utc", "contract_sha256",
                "contract_version", "decision", "evidence_sha256"):
...
def test_repeated_run_idempotency(_mock_git, tmp_repos):
    ...
    canonical, derivative = tmp_repos
    scan = _clean_scan(drift=[
        {"class": "enforcement_gap", "severity": "low",
         "path": "05_governance/gaps.md", "detail": "uncovered rule"},
    ])
    sync = _clean_sync(findings=[
        {"class": "missing_expected_export", "severity": "medium",
         "path": "docs/api.md", "detail": "not exported"},
    ])
    ...
    for i in range(1, len(results)):
        assert results[0]["decision"] == results[i]["decision"], \
            f"Decision diverged on run {i+1}"
        assert results[0]["exit_code"] == results[i]["exit_code"], \
            f"Exit code diverged on run {i+1}"
        assert results[0]["findings_json"] == results[i]["findings_json"], \
            f"Findings diverged on run {i+1}"
