...
def tmp_source(tmp_path: Path) -> Path:
    ...
    (core / "src").mkdir()
    (core / "src" / "validator.py").write_text(
...
class TestRunExport:
    ...
    def test_public_module_full_export(
        self, tmp_source: Path, tmp_target: Path, policy: ExportPolicy
    ) -> None:
        result = run_export(tmp_source, tmp_target, policy, dry_run=True)
    ...
    def test_internal_module_restricted_export(
        self, tmp_source: Path, tmp_target: Path, policy: ExportPolicy
    ) -> None:
        result = run_export(tmp_source, tmp_target, policy, dry_run=True)
        exported_paths = {e["path"] for e in result.exported}
    ...
    def test_actual_export_writes_files(
        self, tmp_source: Path, tmp_target: Path, policy: ExportPolicy
    ) -> None:
    ...
    def test_oversized_file_excluded(
        self, tmp_source: Path, tmp_target: Path, policy_file: Path
    ) -> None:
    ...
    def test_denied_prefix_excluded(
        self, tmp_source: Path, tmp_target: Path, policy: ExportPolicy
    ) -> None:
        result = run_export(tmp_source, tmp_target, policy, dry_run=True)
        excluded_reasons = {
            ...
            if e["path"].startswith("02_audit_logging")
        }
...
class TestManifest:
    def test_manifest_has_required_fields(
        ...
        required = [
            "schema_version", "generated_utc", "status", "source_repo",
            "source_ref", "target_repo", "target_ref", "policy_file",
            "policy_version", "policy_sha256", "tool_version", "files",
            "excluded_files", "bundle_sha256",
        ]
    ...
    def test_manifest_status_pass_when_clean(
        self, tmp_source: Path, tmp_target: Path, policy: ExportPolicy
    ) -> None:
        result = run_export(tmp_source, tmp_target, policy, dry_run=True)
        manifest = generate_manifest(result, policy)
