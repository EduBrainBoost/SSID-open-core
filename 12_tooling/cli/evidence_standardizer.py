...
class EvidenceBundle:
    ...
    release_backup_path: str | None = None
...
class EvidenceStandardizer:
    ...
    def _detect_tool_from_paths(self, changed_paths: list[str]) -> str | None:
    ...
    def _check_sot_sync_passed(self, repo_root: Path) -> bool:
        ...
        return all((repo_root / sot_file).exists() for sot_file in sot_files)
    ...
    def write_standardized_bundle(self, bundle: EvidenceBundle, output_dir: Path) -> Path:
        ...
        bundle_manifest.write_text(json.dumps(asdict(bundle), indent=2, sort_keys=True), encoding="utf-8")
        ...
        verification = {
            ...
            "compliance_status": {
                ...
                "overall": "PASS"
                if all([bundle.duplicate_guard_passed, bundle.write_gate_passed, bundle.sot_sync_passed])
                else "FAIL",
            },
            "gate_summary": {
        ...
        verification_file.write_text(json.dumps(verification, indent=2, sort_keys=True), encoding="utf-8")
...
def main() -> int:
    ...
    output_dir = (
        ...
        if args.output_dir
        else evidence_dir.parent / f"standardized_{evidence_dir.name}"
    )
    ...
    try:
        ...
        print(
            f"COMPLIANCE_STATUS: {bundle.duplicate_guard_passed and bundle.write_gate_passed and bundle.sot_sync_passed}"
        )
