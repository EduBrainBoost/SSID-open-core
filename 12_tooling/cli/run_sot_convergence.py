...
import argparse, hashlib, json, subprocess, sys, traceback, uuid
...
from validation.convergence_manifest_gen import generate_manifest, render_report, SchemaError
...
_REPORT_REQUIRED = ("report_version", "run_identity", "generated_at_utc", "exit_code",
                    "exit_label", "decision", "summary", "findings")
...
def _normalize(raw: Dict[str, Any], source: str, idx: int) -> Dict[str, Any]:
    ...
    entry = {"id": f"{source}-{idx:04d}", "class": raw.get("class", "unknown"),
             "severity": raw.get("severity", "info"), "source": source,
             "path": raw.get("path", ""), "details": detail_text,
...
def _finding(rule_id: str, cls: str, severity: str, path: str, detail: str,
             repo: str = "", ts: str = "", evidence_hash: str = "") -> Dict[str, Any]:
    """Construct a policy finding with full traceability fields."""
    if not ts:
        ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    raw = {"class": cls, "severity": severity, "path": path, "detail": detail,
           "repo": repo, "timestamp": ts, "evidence_hash": evidence_hash,
           "rule_id": rule_id}
...
def _eval_convergence(scan: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if scan.get("status") == "FAIL":
        _a(_finding("D-001", "policy_deny", "critical", "",
                     f"CONVERGENCE_DENY: repo {repo_name} reported status FAIL",
                     repo=repo_name, ts=ts))
    ...
    if repo_role == "canonical" and missing:
        _a(_finding("D-002", "policy_deny", "critical", "",
                     f"CONVERGENCE_DENY: canonical repo {repo_name} has "
                     f"{len(missing)} missing artifacts: {missing}",
                     repo=repo_name, ts=ts))
    ...
    for f in drift:
        ...
        if c == "protected_scope_attempt":
            _a(_finding("D-003", "policy_deny", "critical", p,
                         f"CONVERGENCE_DENY: protected scope attempt in {p} — "
                         f"{d} (severity: {s})",
                         repo=repo_name, ts=ts))

        # D-004: Critical-severity drift findings are denied
        if s == "critical":
            _a(_finding("D-004", "policy_deny", "critical", p,
                         f"CONVERGENCE_DENY: critical drift in {p} — "
                         f"{d} (class: {c})",
                         repo=repo_name, ts=ts))

        # W-003: High-severity drift findings
        if s == "high":
            _a(_finding("W-003", "policy_warn", "high", p,
                         f"CONVERGENCE_WARN: high-severity drift in {p} — "
                         f"{d} (class: {c})",
                         repo=repo_name, ts=ts))

        # W-004: Stale derivative binding
        if c == "stale_derivative_binding":
            _a(_finding("W-004", "policy_warn", "medium", p,
                         f"CONVERGENCE_WARN: stale derivative binding at {p} — {d}",
                         repo=repo_name, ts=ts))

        # I-001: Enforcement gap findings surfaced as info
        if c == "enforcement_gap":
            _a(_finding("I-001", "policy_info", "info", p,
                         f"CONVERGENCE_INFO: enforcement gap at {p} — "
                         f"{d} (severity: {s})",
                         repo=repo_name, ts=ts))

        # I-002: Medium drift findings as info
        if s == "medium":
            _a(_finding("I-002", "policy_info", "info", p,
                         f"CONVERGENCE_INFO: medium drift in {p} — "
                         f"{d} (class: {c})",
                         repo=repo_name, ts=ts))
        ...
        if s == "low":
            _a(_finding("I-002", "policy_info", "info", p,
                         f"CONVERGENCE_INFO: low drift in {p} — "
                         f"{d} (class: {c})",
                         repo=repo_name, ts=ts))

    # W-001: Any drift findings present trigger a warning
    if drift:
        _a(_finding("W-001", "policy_warn", "medium", "",
    ...
    if repo_role == "derivative" and scan.get("export_ready") is False:
        _a(_finding("W-002", "policy_warn", "medium", "",
...
def _eval_derivation(sync: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if sync.get("status") == "fail":
        _a(_finding("D-005", "policy_deny", "critical", "",
                     f"DERIVATION_DENY: overall sync status is FAIL for "
                     f"{canonical_repo} -> {derivative_repo}",
                     repo=derivative_repo, ts=ts))
    ...
    for f in sync.get("findings", []):
        ...
        if c == "forbidden_export":
            _a(_finding("D-001", "policy_deny", "critical", p,
                         f"DERIVATION_DENY: forbidden artifact leaked to derivative — "
                         f"{d} (path: {p})",
                         repo=derivative_repo, ts=ts))

        # D-002: Contract hash mismatch is unconditional deny
        if c == "contract_hash_mismatch":
            _a(_finding("D-002", "policy_deny", "critical", p,
        ...
        if c == "stale_derivative_binding" and s == "critical":
            _a(_finding("D-003", "policy_deny", "critical", p,
                         f"DERIVATION_DENY: critical stale derivative binding — "
                         f"{d} (path: {p})",
                         repo=derivative_repo, ts=ts))
        ...
        if c == "missing_expected_export":
            _a(_finding("W-001", "policy_warn", "medium", p,
                         f"DERIVATION_WARN: expected public artifact missing from "
                         f"derivative — {d} (path: {p})",
                         repo=derivative_repo, ts=ts))

        # W-002: Registry binding missing or inconsistent
        if c == "registry_binding_missing":
            _a(_finding("W-002", "policy_warn", "medium", p,
        ...
        if c == "stale_derivative_binding" and s != "critical":
            _a(_finding("W-004", "policy_warn", "medium", p,
                         f"DERIVATION_WARN: stale derivative binding — "
                         f"{d} (path: {p}, severity: {s})",
                         repo=derivative_repo, ts=ts))

        # W-005: Unsanctioned public artifacts
        if c == "unsanctioned_public_artifact":
            _a(_finding("W-005", "policy_warn", "medium", p,
                         f"DERIVATION_WARN: unsanctioned artifact in derivative — "
                         f"{d} (path: {p})",
                         repo=derivative_repo, ts=ts))

    # W-003: Registry binding status is inconsistent at manifest level
    if sync.get("registry_binding_status") == "inconsistent":
        _a(_finding("W-003", "policy_warn", "medium", "",
                     f"DERIVATION_WARN: registry bindings inconsistent between "
                     f"{canonical_repo} and {derivative_repo}",
                     repo=derivative_repo, ts=ts))
    ...
    if missing_exports and not forbidden_exports:
        _a(_finding("I-001", "policy_info", "info", "",
    ...
    if not missing_exports and not forbidden_exports:
        _a(_finding("I-002", "policy_info", "info", "",
                     f"DERIVATION_INFO: full export completeness — all "
                     f"{len(allowed_exports)} allowed artifacts present",
                     repo=derivative_repo, ts=ts))

    # I-003: Registry binding status is unknown
    if sync.get("registry_binding_status") == "unknown":
        _a(_finding("I-003", "policy_info", "info", "",
                     "DERIVATION_INFO: registry binding status unknown — "
                     "canonical registry may be absent",
                     repo=derivative_repo, ts=ts))

    # I-004: Derivation mode for audit record
    derivation_mode = sync.get("derivation_mode", "unknown")
    _a(_finding("I-004", "policy_info", "info", "",
                 f"DERIVATION_INFO: derivation mode is '{derivation_mode}' for "
                 f"{canonical_repo} -> {derivative_repo}",
                 repo=derivative_repo, ts=ts))
...
def _step_scan(canonical: Path) -> Dict[str, Any]:
    return scanner_scan(canonical_contract_path=_CONTRACT_REL,
                        target_repo_root=str(canonical), repo_role="canonical")
...
def _decide(findings: List[Dict[str, Any]]) -> tuple[str, int]:
    ...
    if any(f.get("class", "").startswith("policy_deny") or
           f["severity"] in ("critical", "high") for f in findings):
        return "fail", EXIT_DENY
    if any(f["severity"] in ("medium", "low") or
           f.get("class", "").startswith("policy_warn") for f in findings):
        return "warn", EXIT_WARN
    return "pass", EXIT_SUCCESS


def _run_identity(canonical: Path, derivative: Path, findings: List,
                  decision: str, report_str: str) -> Dict[str, Any]:
...
def _json_report(scan: Dict, manifest: str, sync: Dict, findings: List,
                 decision: str, exit_code: int, rid: Dict) -> Dict[str, Any]:
    lbl = {EXIT_SUCCESS: "success", EXIT_WARN: "warn", EXIT_DENY: "deny",
           EXIT_SYSTEM_ERROR: "system_error"}.get(exit_code, "unknown")
    return {
        "report_version": "2.0.0", "run_identity": rid,
        "generated_at_utc": rid["timestamp_utc"],
        "pipeline_steps": ["scan", "manifest", "opencore_sync", "policy_check", "report"],
        "exit_code": exit_code, "exit_label": lbl, "decision": decision,
        "summary": {"total_findings": len(findings), "max_severity": _max_sev(findings),
                     "scanner_status": scan.get("status", "UNKNOWN"),
                     "opencore_sync_status": sync.get("status", "unknown")},
        "findings": findings, "scanner_result": scan, "opencore_sync_result": sync,
    }
...
def _md_report(rpt: Dict[str, Any], scan: Dict[str, Any]) -> str:
    ...
    lines += [f"# SoT Convergence Report -- {lbl}", "",
              f"Generated: {rpt['generated_at_utc']}", f"Decision: **{rpt['decision']}**", "",
              "## Run Identity", "", "| Field | Value |", "|-------|-------|"]
    lines += [f"| {k} | `{v}` |" for k, v in rid.items()]
    lines += ["", "## Pipeline Summary", "", "| Field | Value |", "|-------|-------|",
    ...
    if ff:
        lines += ["", "## Unified Findings", "",
                   ...
                   "|----|--------|-------|----------|------|---------|"]
        lines += [f"| {f['id']} | {f['source']} | `{f['class']}` | {f['severity']}"
                  f" | `{f['path']}` | {f['details']} |" for f in ff]
    else:
...
def run(args: argparse.Namespace) -> int:
    ...
    report = _json_report(scan_result, manifest_json, sync_result,
                          findings, decision, exit_code, rid)
    ...
    if validation_errors:
        ...
        audit_err = {"event_type": "schema_validation_failure", "errors": validation_errors,
                     "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")}
        (output_dir / "schema_error_audit.json").write_text(
            json.dumps(audit_err, indent=2) + "\n", encoding="utf-8")
        return EXIT_SYSTEM_ERROR

    # Write outputs
    (output_dir / "sot_convergence_report.json").write_text(report_str + "\n", encoding="utf-8")
    (output_dir / "sot_convergence_report.md").write_text(
        _md_report(report, scan_result), encoding="utf-8")
    (output_dir / "sot_convergence_report.sha256").write_text(
        f"{rid['evidence_sha256']}  sot_convergence_report.json\n", encoding="utf-8")

    audit = {"event_type": "sot_convergence_run", "run_identity": rid,
             "decision": decision, "exit_code": exit_code,
             "total_findings": len(findings), "max_severity": _max_sev(findings),
             "policy_findings_count": len(policy_findings)}
    audit_path = output_dir / "run_audit_event.json"
    audit_path.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    for lbl, val in [("JSON report", output_dir / "sot_convergence_report.json"),
                     ("MD report", output_dir / "sot_convergence_report.md"),
                     ("Audit event", audit_path), ("SHA-256", rid["evidence_sha256"]),
                     ("Decision", decision), ("Exit code", f"{exit_code} ({report['exit_label']})")]:
