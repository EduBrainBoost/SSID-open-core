#!/usr/bin/env python3
"""Readiness Verification Tool — comprehensive integrity check suite.

Runs a verification suite to assess whether the SSID repository
meets technical readiness criteria:
...
Output: JSON report with per-check results and overall assessment.
...
NOTE: This tool verifies technical readiness criteria. Deployment decisions
require additional approval and operational authorization beyond this check.
"""
...
def _scan_files(
    repo: Path, patterns: list[re.Pattern], skip_dirs: set[str] | None = None, extensions: set[str] | None = None
) -> list[dict[str, Any]]:
...
class ProductionReadinessChecker:
    ...
    def check_all_tests_green(self) -> ReadinessCheck:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return self._add("all_tests_green", "testing", "pass", f"{count} tests passed")
            else:
                # Extract failure info
                lines = (result.stdout + result.stderr).strip().split("\n")
                summary = lines[-1] if lines else "pytest failed"
                return self._add("all_tests_green", "testing", "fail", f"pytest failed: {summary[:200]}")
        except subprocess.TimeoutExpired:
            return self._add("all_tests_green", "testing", "fail", "pytest timed out after 300s")
        except FileNotFoundError:
            return self._add("all_tests_green", "testing", "fail", "pytest not found")
    ...
    def check_secret_scan_clean(self) -> ReadinessCheck:
        ...
        if not findings:
            return self._add("secret_scan_clean", "security", "pass", "no plaintext secrets detected")
        ...
        return self._add(
            ...
            "security",
            "fail",
            f"{len(findings)} potential secret(s) in {len(files)} file(s): {', '.join(files[:5])}",
        )
    ...
    def check_pii_scan_clean(self) -> ReadinessCheck:
        ...
        if not findings:
            return self._add("pii_scan_clean", "security", "pass", "no PII patterns detected")
        files = sorted(set(f["file"] for f in findings))
        return self._add(
            "pii_scan_clean",
            "security",
            "warn",
            f"{len(findings)} potential PII pattern(s) in {len(files)} file(s): {', '.join(files[:5])}",
        )
    ...
    def check_charts_complete(self) -> ReadinessCheck:
        ...
        if count >= EXPECTED_CHART_COUNT:
            return self._add(
                "charts_complete", "completeness", "pass", f"{count}/{EXPECTED_CHART_COUNT} charts present"
            )
        elif count >= EXPECTED_CHART_COUNT * 0.9:
            return self._add(
                "charts_complete", "completeness", "warn", f"{count}/{EXPECTED_CHART_COUNT} charts present (>90%)"
            )
        else:
            return self._add(
                "charts_complete", "completeness", "fail", f"{count}/{EXPECTED_CHART_COUNT} charts present"
            )
    ...
    def check_manifests_complete(self) -> ReadinessCheck:
        ...
        if count == EXPECTED_MANIFEST_COUNT:
            return self._add(
                "manifests_complete", "completeness", "pass", f"{count}/{EXPECTED_MANIFEST_COUNT} manifests present"
            )
        else:
            return self._add(
                "manifests_complete",
                "completeness",
                "fail",
                f"{count}/{EXPECTED_MANIFEST_COUNT} manifests present; missing: {', '.join(missing[:5])}",
            )
    ...
    def check_convergence_pass(self) -> ReadinessCheck:
        ...
        if not convergence_script.is_file():
            return self._add("convergence_pass", "consistency", "fail", "convergence_checker.py not found")
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                return self._add("convergence_pass", "consistency", "pass", "convergence checker passed")
            elif result.returncode == 1:
                return self._add("convergence_pass", "consistency", "warn", "convergence checker returned warnings")
            else:
                summary = result.stdout.strip().split("\n")[-1] if result.stdout.strip() else "check failed"
                return self._add(
                    "convergence_pass", "consistency", "fail", f"convergence checker failed: {summary[:200]}"
                )
        except (subprocess.TimeoutExpired, OSError) as exc:
            return self._add("convergence_pass", "consistency", "fail", f"convergence checker error: {exc}")
    ...
    def check_evidence_present(self) -> ReadinessCheck:
        ...
        if found > 0:
            return self._add("evidence_present", "audit", "pass", f"{found} evidence file(s) found")
        else:
            return self._add("evidence_present", "audit", "fail", "no WORM evidence files found")
    ...
    def check_gates_pass(self) -> ReadinessCheck:
        ...
        if not gate_script.is_file():
            return self._add("gates_pass", "gates", "fail", "run_all_gates.py not found")
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                return self._add("gates_pass", "gates", "pass", "all gates passed")
            else:
                summary = result.stdout.strip().split("\n")[-1] if result.stdout.strip() else "gates failed"
                return self._add("gates_pass", "gates", "fail", f"gate failures detected: {summary[:200]}")
        except (subprocess.TimeoutExpired, OSError) as exc:
            return self._add("gates_pass", "gates", "fail", f"gate runner error: {exc}")
    ...
    def check_root24_lock(self) -> ReadinessCheck:
        ...
        if not missing and not extra_dirs:
            return self._add("root24_lock", "structure", "pass", f"ROOT-24-LOCK verified: {len(actual_roots)} roots")
        else:
            parts = []
            if missing:
            ...
            return self._add("root24_lock", "structure", "fail", f"ROOT-24-LOCK violation: {'; '.join(parts)}")
    ...
    def check_no_placeholders(self) -> ReadinessCheck:
        ...
        if not placeholders:
            return self._add("no_placeholders", "completeness", "pass", "no 49-byte placeholder files found")
        else:
            return self._add(
                "no_placeholders",
                "completeness",
                "warn",
                f"{len(placeholders)} potential placeholder(s): {', '.join(placeholders[:5])}",
            )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="SSID Readiness Verification Tool")
    parser.add_argument("--repo", default=str(REPO_ROOT), help="Path to SSID repository root")
    ...
    if args.json:
        ...
        lines = [f"Readiness Verification Report — {report.timestamp}", f"Assessment: {report.verdict}", ""]
