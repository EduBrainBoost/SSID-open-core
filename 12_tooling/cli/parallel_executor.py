...
class ParallelTask:
    ...
    def __post_init__(self):
        if not self.created_utc:
            self.created_utc = datetime.datetime.now(datetime.UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z")
...
class ParallelExecutor:
    ...
    def __init__(self, repo_root: Optional[Path] = None, max_workers: Optional[int] = None):
        self.repo_root = repo_root or REPO_ROOT
        self.dispatcher = DISPATCHER
        self.max_workers = max_workers or min(MAX_CONCURRENT_TASKS, multiprocessing.cpu_count())
        self.active_tasks: Dict[str, Optional[subprocess.Popen]] = {}
    ...
    def _validate_task_spec(self, task_spec_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid task spec: {e}")
    ...
    def _execute_task(self, task: ParallelTask) -> TaskResult:
        ...
        try:
            ...
            temp_spec = self._create_task_spec_with_role(
                self._validate_task_spec(task.task_spec_path),
                role,
                task.tool
            )
    ...
    def _get_ready_tasks(self) -> List[ParallelTask]:
        ...
        for task in self.task_queue:
            if self._check_dependencies(task) and task.task_id not in self.active_tasks and task.task_id not in self.completed_tasks:
...
def main() -> int:
    ...
    parser.add_argument("--tool", choices=["claude", "codex", "gemini", "kilo", "copilot", "opencode"], 
                       help="Tool for single task execution")
    parser.add_argument("--max-workers", type=int, default=MAX_CONCURRENT_TASKS,
