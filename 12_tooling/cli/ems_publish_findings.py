...
_EVENT_REQUIRED = ("event_type", "timestamp", "pipeline_version", "commit_id",
                   "summary", "report_sha256", "findings_preview")
...
def _build_payload(report: dict, report_raw: bytes, commit_id: str,
                   manifest_hash: str, run_identity: dict | None = None) -> dict:
    ...
    payload: dict = {
        ...
        "summary": {"total_findings": len(findings), "deny_count": deny_c,
                     "warn_count": warn_c, "info_count": info_c, "decision": decision},
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--manifest-hash", default="",
                   help="SHA-256 of integrity manifest (auto-derived if omitted)")
    p.add_argument("--run-identity", default=None,
                   help="Path to run-identity JSON (run_id, canonical_commit, etc.)")
    ...
    dry.add_argument("--no-dry-run", dest="dry_run", action="store_false",
                     help="Actually POST the event to EMS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if schema_errors:
        ...
        audit_err = {"event_type": "schema_validation_failure",
                     "schema": _EVENT_SCHEMA, "errors": schema_errors,
